...
def generate_real_estate_data(num_samples=10000):
    ...
    df["type_house"] = np.random.choice(
        type_houses, size=num_samples, p=type_house_probabilities
    )
    ...
    for house_type in type_houses:
        ...
        if house_type == "chung cư":
            df.loc[mask, "type_forrent"] = np.random.choice(
                ["cho thuê", "bán"], size=mask.sum(), p=[0.3, 0.7]
            )
        elif house_type == "nhà nguyên căn":
            df.loc[mask, "type_forrent"] = np.random.choice(
                ["cho thuê", "bán"], size=mask.sum(), p=[0.5, 0.5]
            )
        elif house_type == "phòng trọ":
            df.loc[mask, "type_forrent"] = "cho thuê"
        elif house_type == "văn phòng":
            df.loc[mask, "type_forrent"] = np.random.choice(
                ["cho thuê", "bán"], size=mask.sum(), p=[0.9, 0.1]
            )
    ...
    for house_type in type_houses:
        mask = df["type_house"] == house_type
        if house_type == "chung cư":
            df.loc[mask, "area"] = np.random.choice(
                [65, 70, 75, 84, 90, 120, 124], size=mask.sum()
            )
    ...
    df["furniture"] = np.random.choice(
        ["đầy đủ", "cơ bản", "không có"], size=num_samples
    )
    ...
    df["address"] = (
        df["address_number"].astype(str) + " " + df["street"] + ", " + df["district"]
    )
